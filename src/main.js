...
const PROLIFIC_COMPLETION_URL =
  "https://app.prolific.com/submissions/complete?cc=C1MDRYG9";
...
const previewAssignment = previewAssignmentFromLocation(window.location);
const itemRoute = previewAssignment.itemRoute;
...
if (hasAnyProlific && !isDataCollectionSession) {
  ...
  `);
} else if (isDataCollectionSession && !hasDataPipeExperimentForItem(itemRoute)) {
  renderBlockingScreen(`
    <section class="experiment-column">
      <h1>Study link error</h1>
      <p>This study link does not point to an active version of the study.</p>
      <p>Please return to Prolific and open the study link again.</p>
    </section>
  `);
} else {
...
function renderBlockingScreen(content) {
  root.innerHTML = `
...
function runExperiment() {
  ...
  const restoredRatingOrder = validRatingOrder(storedSession?.ratingOrder)
    ? storedSession.ratingOrder
    : undefined;
  const ratingOrder =
    restoredRatingOrder ?? orderedRatingTypes((values) => jsPsych.randomization.shuffle(values));

  jsPsych.data.addProperties({
    participant_uuid: participantUuid,
    study: "main-experiment",
    study_version: studyVersion(itemRoute),
  ...
  if (isDataCollectionSession) {
    const restoredCondition = conditionFromId(storedSession?.localConditionId, itemRoute);

    if (restoredCondition) {
      applyCondition(
        restoredCondition,
        storedSession.assignmentSource ?? `datapipe-sequential-resume-${itemRoute}`,
      );
    }
  } else {
    applyCondition(previewAssignment.selectedCondition, previewAssignment.assignmentSource);
  }

  persistSession({ participantUuid, ratingOrder });
  ...
  function applyCondition(condition, source) {
    ...
    persistSession({
      ...
      ratingOrder,
      itemRoute,
      localConditionId: selectedCondition.local_condition_id,
      globalConditionId: selectedCondition.global_condition_id,
      assignmentSource,
    });
  }
  ...
  function buildRatingQuestions() {
    return ratingOrder.map((ratingType) => {
  ...
  const conditionAssignment = {
    type: PipePlugin,
    action: "condition",
    experiment_id: dataPipeExperimentIdForItem(itemRoute),
    wait_message: "<p>Preparing the study. Please wait.</p>",
    data: { trial_kind: "condition_assignment" },
    on_finish: (data) => {
      const conditionId = conditionIdFromPipeResult(
        data.result,
        conditionsForItem(itemRoute).length,
      );
      assignmentFailed = conditionId === undefined;
      data.assignment_valid = !assignmentFailed;

      if (!assignmentFailed) {
        applyCondition(conditionFromId(conditionId, itemRoute), `datapipe-sequential-${itemRoute}`);
      }
    },
  };
  ...
  const instructions = {
    ...
    stimulus: `
      <section class="instruction-screen">
        ...
        <p>You will then answer three questions using sliders. There are no right or wrong answers. Please respond based <strong>only</strong> on the information in the description.</p>
  ...
  const ratings = {
    ...
    on_start: (trial) => {
      trial.stimulus = ratingStimulus(selectedCondition);
      trial.questions = buildRatingQuestions();
    },
  };
  ...
  const saveData = {
    ...
    action: "save",
    experiment_id: dataPipeExperimentIdForItem(itemRoute),
    filename: dataFilename(itemRoute, submissionId),
...
function validRatingOrder(order) {
  return (
    Array.isArray(order) &&
    order.length === RATING_TYPES.length &&
    order[order.length - 1] === "NAT" &&
    new Set(order).size === RATING_TYPES.length &&
    order.every((ratingType) => RATING_TYPES.includes(ratingType))
  );
}
